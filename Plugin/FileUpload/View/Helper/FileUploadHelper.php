...
/**
  ...
  * @property FileUploadHelper $FileUploadHelper
  *
  * @example
  ...
  function input($options = array()){
    ...
    if($configs['model']){
      unset($options['model'], $options['var']);
        pr($configs);
        pr(array_merge(array('type'=>'file'), $options));
        echo "{$configs['model']}.".$this->inputCount++.".{$configs['var']}";
        //return $this->Form->input("{$configs['model']}.".$this->inputCount++.".{$configs['var']}", array_merge(array('type'=>'file'), $options));
        $this->Form->input('Upload.0.file');
    }
    else {
